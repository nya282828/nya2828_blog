...
## DOM構造以外の条件指定に関する期待
現状CSSにおける条件分岐はDOM構造のコンテキストによる条件しか書けない。しかしかつてはそれも選択肢はかなり少なかったが、昔に比べて利用可能な選択はかなり広がってとても楽になった。今後CSSにカスタム関数やif文が追加されれば、DOM構造以外にもプロパティ値や変数の比較で条件を作れるようになり、さらに簡単かつ複雑なコンテキスト下での処理も可能になることを期待したい。